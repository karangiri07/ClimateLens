...
const CITIES_DATA: Record<string, { temp: number; condition: string; humidity: number; wind: number; aqi: number; uv: number; forecast: { day: string; date: string; max: number; min: number; cond: string }[] }> = {
  Virar: {
    ...
    uv: 7,
    forecast: [
      { day: 'Mon', date: '15 Sep', max: 29, min: 24, cond: 'Partly Cloudy' },
      { day: 'Tue', date: '16 Sep', max: 30, min: 24, cond: 'Sunny' },
      { day: 'Wed', date: '17 Sep', max: 29, min: 23, cond: 'Light Rain' },
      { day: 'Thu', date: '18 Sep', max: 28, min: 23, cond: 'Cloudy' },
      { day: 'Fri', date: '19 Sep', max: 30, min: 24, cond: 'Sunny' },
      { day: 'Sat', date: '20 Sep', max: 29, min: 24, cond: 'Showers' },
      { day: 'Sun', date: '21 Sep', max: 30, min: 24, cond: 'Partly Cloudy' },
    ]
  },
  Pune: {
    temp: 27,
    ...
    <div className={`min-h-screen w-full max-w-full overflow-x-hidden font-sans transition-colors duration-200 ${
      ...
      <header className={`sticky top-0 z-50 w-full border-b backdrop-blur-xl transition-colors ${
        ...
        <div className="w-full px-4 sm:px-6 lg:px-8 xl:px-10">
          <div className="min-h-[68px] flex items-center justify-between gap-3 xl:gap-6">
            ...
            <button
              ...
              className="flex items-center gap-2.5 sm:gap-3 shrink-0 text-left group"
              aria-label="Go to ClimateLens home"
            >
              {/* ClimateLens mark */}
              <span className="relative w-10 h-10 sm:w-11 sm:h-11 shrink-0 rounded-[14px] overflow-hidden bg-gradient-to-br from-emerald-600 via-teal-600 to-cyan-700 text-white shadow-lg shadow-emerald-700/20 transition-transform duration-200 group-hover:scale-[1.04]">
                <span className="absolute -top-3 -right-3 w-8 h-8 rounded-full bg-white/20 blur-sm" />
                <svg viewBox="0 0 44 44" className="relative w-full h-full p-2.5" fill="none" aria-hidden="true">
                  <circle cx="22" cy="22" r="14.5" stroke="currentColor" strokeWidth="1.8" opacity="0.9" />
                  <path d="M8.5 22h27M22 7.5c4 4.2 6.1 9.1 6.1 14.5S26 32.3 22 36.5M22 7.5c-4 4.2-6.1 9.1-6.1 14.5S18 32.3 22 36.5" stroke="currentColor" strokeWidth="1.3" opacity="0.5" />
                  <path d="M13 27.5c3.8-1.1 7.3-3.8 9.4-8.5 1.8 1.2 4.5 1.7 7.2 1.2-1.1 6.4-5.5 10.5-11.1 10.9-2.3.2-4.3-.9-5.5-3.6Z" fill="white" />
                  <path d="M15 29.4c3.1-1.1 6-3.4 8.4-7.1" stroke="#10B981" strokeWidth="1.5" strokeLinecap="round" />
                </svg>
              </span>

              <span className="min-w-0">
                <span className={`block text-[18px] sm:text-xl font-black tracking-tight leading-none ${
                  isDarkMode ? 'text-white' : 'text-slate-950'
                }`}>
                  Climate<span className="text-emerald-500">Lens</span>
                </span>
                <span className="hidden sm:block text-[9px] font-bold text-slate-400 uppercase tracking-[0.16em] mt-1 whitespace-nowrap">
            ...
            <nav className="hidden lg:flex items-center justify-center gap-1 text-[13px] font-semibold flex-1">
              {['Home', 'Weather', 'Climate Data', 'Explore Map', 'Learn', 'Take Action', 'About'].map((tab) => (
                <button
                  ...
                  className={`relative px-3 py-2.5 rounded-xl transition-all ${
                    activeTab === tab
                      ? isDarkMode
                ...
                >
                  {tab}
                  {activeTab === tab && (
                    <span className="absolute left-1/2 -translate-x-1/2 bottom-1 w-5 h-0.5 rounded-full bg-emerald-500" />
                  )}
                </button>
              ))}
            </nav>
            ...
            <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                ...
                <input
                  ...
                  className={`w-full text-xs py-2.5 pl-4 pr-10 rounded-full border outline-none transition-all ${
                    isDarkMode
                      ...
                      : 'bg-slate-100 border-slate-200 text-slate-800 placeholder:text-slate-400 hover:bg-white focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/15'
                  }`}
                />
              ...
              <div className={`hidden sm:flex items-center p-1 rounded-full border ${
                ...
                <button
                  ...
                  className={`p-1.5 rounded-full transition ${
                    isDarkMode ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-700'
                  }`}
                  title="Dark Mode"
            ...
            <div className="px-4 sm:px-6 py-4 space-y-3">
              ...
              <div className="md:hidden relative">
                <input
                  ...
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className={`w-full text-sm py-3 pl-4 pr-10 rounded-2xl border outline-none ${
                    isDarkMode
                      ? 'bg-slate-900 border-slate-700 text-white placeholder:text-slate-500'
                      : 'bg-slate-50 border-slate-200 text-slate-800 placeholder:text-slate-400'
                  }`}
                />
                <SearchIcon className="absolute right-4 top-3.5 text-slate-400 w-4 h-4 pointer-events-none" />
              </div>
              ...
              <nav className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {['Home', 'Weather', 'Climate Data', 'Explore Map', 'Learn', 'Take Action', 'About'].map((tab) => (
                  <button
                    ...
                    className={`min-h-11 px-3 rounded-xl text-xs font-bold text-left transition ${
                      activeTab === tab
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-1">
                ...
                <button
                  type="button"
                  onClick={() => {
                    setActiveTab('Take Action');
                    setMobileMenuOpen(false);
                  }}
                  className="flex-1 sm:flex-none sm:ml-auto px-5 py-3 rounded-xl bg-emerald-700 hover:bg-emerald-600 text-white font-bold text-xs transition"
                >
                  Take Action →
                </button>
              </div>
      ...
      {activeTab === 'Home' && (
        <>
          ...
          <section className="relative overflow-hidden bg-gradient-to-br from-emerald-950 via-teal-900 to-slate-900 text-white py-12 sm:py-14 lg:py-20 px-4 sm:px-6 lg:px-8 xl:px-10">
            <div 
            ...
            <div className="w-full max-w-[1600px] mx-auto relative z-10 grid grid-cols-1 xl:grid-cols-12 gap-8 lg:gap-10 items-center">
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-semibold text-slate-400">Current demo conditions</p>
                      </div>
                    </div>
                  </div>
          ...
          <main className="w-full max-w-[1600px] mx-auto px-4 sm:px-8 lg:px-12 py-10 space-y-10">
            ...
            <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              <div className={`rounded-2xl p-5 border shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
        ...
        <main className="w-full max-w-[1600px] mx-auto px-4 sm:px-8 lg:px-12 py-10 space-y-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-6 border-slate-200 dark:border-slate-800">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">Interactive weather parameters and multi-day forecast examples for Maharashtra cities.</p>
            </div>
          ...
          <div className={`p-8 rounded-3xl border grid grid-cols-1 lg:grid-cols-3 gap-7 lg:gap-8 items-center ${
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-slate-400">{cityData.condition} · Demo dataset</p>
...
{activeTab === 'Explore Map' && (
  <main className="w-full px-4 sm:px-8 lg:px-12 py-10 space-y-8">
    ...
    <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-6">
      ...
      {/* Layer Controls */}
      <div
        className={`p-1.5 rounded-2xl border flex flex-wrap sm:flex-nowrap items-center gap-1 ${
          isDarkMode
    ...
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
      ...
      {/* MAP */}
      <div
        className={`xl:col-span-8 min-h-[440px] sm:min-h-[560px] rounded-[2rem] border overflow-hidden relative ${
          isDarkMode
...
{activeTab === 'Take Action' && (
  <main className="w-full max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-7 sm:py-8 space-y-6">
    {/* Hero Banner */}
    <div
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
      ...
      <div
        className={`lg:col-span-4 p-6 sm:p-8 rounded-3xl flex flex-col items-center justify-center text-center space-y-6 ${
          isDarkMode
    ...
    <div className={`rounded-3xl border p-5 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${
      isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'
    }`}>
      <div>
        <p className="text-xs font-black uppercase tracking-widest text-emerald-600">Community</p>
        <h3 className="text-lg font-extrabold mt-1">Want to help your local climate initiative?</h3>
        <p className="text-xs text-slate-400 mt-1">Join a local drive and connect with community activities.</p>
      </div>
      <button
        type="button"
        onClick={() => setShowVolunteerModal(true)}
        className="shrink-0 px-5 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs transition"
      >
        Join a Local Drive →
      </button>
    </div>
      ...
      {activeArticle && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className={`w-full max-w-2xl max-h-[88vh] overflow-y-auto rounded-3xl p-4 sm:p-8 space-y-5 sm:space-y-6 border ${
            ...
            <img src={activeArticle.image} alt={activeArticle.title} className="w-full h-44 sm:h-56 object-cover rounded-2xl" />
